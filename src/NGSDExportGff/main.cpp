...
public:
    ...
    virtual void main()
    {
        ...
        // /mnt/users/ahdoebm1/ngs-bits/bin/GenesToGff -out /mnt/users/ahdoebm1/test.gff3
        ...
        for (QHash<QString, QByteArray> gene : genes) {
			...
			QByteArray geneId = "gene:" + gene["ensembl_id"];
			QByteArray info = "";
			info += "ID=" + geneId + ";";
            ...
            for(QHash<QString, QByteArray> transcript : currentTranscripts) {
				...
				QByteArray transcriptId = "transcript:" + transcript["name"]; // enst...
				info = "";
