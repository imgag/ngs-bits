...
private:
	...
	TEST_METHOD(removeDuplicates_VCF)
	{
		VcfFile vl,vl2;
	...
	TEST_METHOD(loadFromVCF_noSampleOrFormatColumn)
	{
		VcfFile vl;
	...
	TEST_METHOD(loadFromVCF_undeclaredAnnotations)
	{
		VcfFile vl;
	...
	TEST_METHOD(loadFromVCF_emptyFormatAndInfo)
	{
	...
	TEST_METHOD(checkThatEmptyVariantAnnotationsAreFilled)
	{
	...
	TEST_METHOD(storeAfterAddingSample)
	{
		VcfFile vl;
		vl.load(TESTDATA("data_in/sort_in.vcf"));
		vl.sort();
	...
	TEST_METHOD(storeAfterAddingThreeSamples)
	{
		VcfFile vl;
		vl.load(TESTDATA("data_in/sort_in.vcf"));
		vl.sort();
	...
	TEST_METHOD(copyMetaData_check_sampleInfoFormatPtrs)
	{
		VcfFile vcf_file;
    ...
    TEST_METHOD(convertToStringAndParseFromString)
    {
        VcfFile vcf_file;
    ...
    TEST_METHOD(loadStoreComparison)
    {
        VcfFile vcf_file;
		vcf_file.load(TESTDATA("data_in/panel_vep.vcf"));
