...
TEST_CLASS(QcRuleMatcher_Test)
{
	...
	TEST_METHOD(checkEvaluation)
	{
		...
		QString quality = qc_matcher.evaluate(name_short, sys_type, qc_data, false);
		S_EQUAL(quality, "bad");
		...
		qc_data.insert(QCValue("target region read depth", 30.0, "", "QC:2000025"));
		quality = qc_matcher.evaluate(name_short, sys_type, qc_data, false);
		...
		quality = qc_matcher.evaluate(name_short, sys_type, "target region 20x percentage", 99.0, false);
		S_EQUAL(quality, "good");

		// Single QC value - medium
		quality = qc_matcher.evaluate(name_short, sys_type, "target region 20x percentage", 95.0, false);
		S_EQUAL(quality, "medium");

		// Single QC value - bad
		quality = qc_matcher.evaluate(name_short, sys_type, "target region 20x percentage", 89.0, false);
		S_EQUAL(quality, "bad");

		// Single QC value - good ("name short" instead of type)
		name_short = "RPGR-Ex15";
		quality = qc_matcher.evaluate(name_short, sys_type, "target region read depth", 1100.0, false);
		S_EQUAL(quality, "good");

		// Single QC value - medium  ("name short" instead of type)
		quality = qc_matcher.evaluate(name_short, sys_type, "target region read depth", 450.0, false);
		S_EQUAL(quality, "medium");
	}
};