...
DBTable NGSD::processedSampleSearch(const ProcessedSampleSearchParameters& p)
{
	...
	if (p.add_dates)
	{
		...
		fields << "s.received as year_received";
...
SampleData NGSD::getSampleData(const QString& sample_id)
{
	//execute query
	SqlQuery query = getQuery();
	query.exec("SELECT s.name, s.name_external, s.gender, s.quality, s.comment, s.disease_group, s.disease_status, s.tumor, s.ffpe, s.sample_type, s.sender_id, s.species_id, s.received, s.receiver_id, s.tissue, s.patient_identifier, s.year_of_birth, s.sampling_date  FROM sample s WHERE id=" + sample_id);
	if (query.size()==0)
	{
	...
	QVariant sampling_date = query.value(17);
	if (!sampling_date.isNull())
	{
		output.sampling_date = sampling_date.toString();
	}
