	...
	while(query.next())
	{
		...
		switch(permission)
		{
			case Permission::PROJECT:
                ps_ids += NGSHelper::listToSet(getValuesInt("SELECT id FROM processed_sample WHERE project_id=" + data.toString()));
				break;
			case Permission::PROJECT_TYPE:
                ps_ids += NGSHelper::listToSet(getValuesInt("SELECT ps.id FROM processed_sample ps, project p WHERE ps.project_id=p.id AND p.type='" + data.toString() + "'"));
				break;
			case Permission::SAMPLE:
                ps_ids += NGSHelper::listToSet(getValuesInt("SELECT id FROM processed_sample WHERE sample_id=" + data.toString()));
				break;
			case Permission::STUDY:
                ps_ids += NGSHelper::listToSet(getValuesInt("SELECT processed_sample_id FROM study_sample WHERE study_id=" + data.toString()));
				break;
		}
	}
...
QSet<int> NGSD::getRNACohort(int sys_id, const QString& tissue_type, const QString& project, const QString& ps_id, RnaCohortDeterminationStategy cohort_type, const QByteArray& mode, const QStringList& exclude_quality, bool debug)
{
	...
	if (mode == "genes")
	{
        all_ps_ids = NGSHelper::listToSet(getValuesInt("SELECT DISTINCT e.processed_sample_id FROM expression e"));
	}
	else if (mode == "exons")
	{
        all_ps_ids = NGSHelper::listToSet(getValuesInt("SELECT DISTINCT e.processed_sample_id FROM expression_exon e"));
	}
	else
	{
	...
	if ((cohort_type == RNA_COHORT_GERMLINE) || (cohort_type == RNA_COHORT_GERMLINE_PROJECT))
	{
        ...
        cohort = NGSHelper::listToSet(getValuesInt(query_string_cohort));
