...
{	
    ...
    output = output.fromText(ApiCaller().post("low_coverage_regions", RequestUrlParams(), HttpHeaders(), QString("roi="+bed_file.toText().toUtf8()+"&bam_url_id="+bam_file_parts[bam_file_parts.count()-2]+"&cutoff="+QString::number(cutoff)).toUtf8(), true, true));
