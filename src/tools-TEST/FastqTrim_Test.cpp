...
private:
	...
	TEST_METHOD(start)
	{
		...
		COMPARE_FILES("out/FastqTrim_out1.fastq.gz", TESTDATA("data_out/FastqTrim_out1.fastq.gz"));
	}
	...
	TEST_METHOD(start_end)
	{
		...
		COMPARE_FILES("out/FastqTrim_out2.fastq.gz", TESTDATA("data_out/FastqTrim_out2.fastq.gz"));
	}
	...
	TEST_METHOD(start_len)
	{
		...
		COMPARE_FILES("out/FastqTrim_out3.fastq.gz", TESTDATA("data_out/FastqTrim_out3.fastq.gz"));
	}

	TEST_METHOD(max_len)
	{
		EXECUTE("FastqTrim", "-in " + TESTDATA("data_in/FastqTrim_in1.fastq.gz") + " -out out/FastqTrim_out4.fastq.gz -end 5 -max_len 80");
		COMPARE_FILES("out/FastqTrim_out4.fastq.gz", TESTDATA("data_out/FastqTrim_out4.fastq.gz"));
	}
	...
	TEST_METHOD(all)
	{
		...
		COMPARE_FILES("out/FastqTrim_out5.fastq.gz", TESTDATA("data_out/FastqTrim_out5.fastq.gz"));
	}
	...
	TEST_METHOD(long_read)
	{
		...
		COMPARE_FILES("out/FastqTrim_out6.fastq.gz", TESTDATA("data_out/FastqTrim_out6.fastq.gz"));
	}
