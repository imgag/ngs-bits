...
private:
    ...
    {
		EXECUTE("GenePrioritization", "-in " + TESTDATA("data_in/GenePrioritization_in.tsv") + " -graph " + TESTDATA("data_in/GenePrioritization_graph.tsv") + " -out out/GenePrioritization_out2.tsv -method random_walk -test_mode");
        IS_TRUE(QFile::exists("out/GenePrioritization_out2.tsv"));
		if(Helper::isMacOS()) // random number generator behaves differently under different operating systems
        {
