...
private:
	...
	TEST_METHOD(default_test)
	{
		...
		COMPARE_FILES("out/FastqConcat_out.fastq.gz", TESTDATA("data_out/FastqConcat_out.fastq.gz"));
	}
	...
	TEST_METHOD(long_test)
	{
		...
		COMPARE_FILES("out/FastqConcat_out2.fastq.gz", TESTDATA("data_out/FastqConcat_out2.fastq.gz"));
	}
