...
private:
	...
	TEST_METHOD(test_panel)
	{
		...
		REMOVE_LINES(lastLogFile(), QRegularExpression("^WARNING: transactions"));
		REMOVE_LINES(lastLogFile(), QRegularExpression("^filename:"));
		COMPARE_FILES(lastLogFile(), TESTDATA("data_out/NGSDAddVariantsGermline_out1.log"));
		...
		REMOVE_LINES(lastLogFile(), QRegularExpression("^WARNING: transactions"));
		REMOVE_LINES(lastLogFile(), QRegularExpression("^filename:"));
		COMPARE_FILES(lastLogFile(), TESTDATA("data_out/NGSDAddVariantsGermline_out2.log"));
		...
		REMOVE_LINES(lastLogFile(), QRegularExpression("^WARNING: transactions"));
		REMOVE_LINES(lastLogFile(), QRegularExpression("^filename:"));
		COMPARE_FILES(lastLogFile(), TESTDATA("data_out/NGSDAddVariantsGermline_out3.log"));
	...
	TEST_METHOD(test_wes)
	{
		...
		REMOVE_LINES(lastLogFile(), QRegularExpression("^WARNING: transactions"));
		REMOVE_LINES(lastLogFile(), QRegularExpression("^filename:"));
		COMPARE_FILES(lastLogFile(), TESTDATA("data_out/NGSDAddVariantsGermline_out4.log"));
	...
	TEST_METHOD(sv_default_import)
	{
		...
		//check log
		REMOVE_LINES(lastLogFile(), QRegularExpression("^filename:"));
		COMPARE_FILES(lastLogFile(), TESTDATA("data_out/NGSDAddVariantsGermline_out5.log"));
	}


	TEST_METHOD(sv_reimport)
	{
		if (!NGSD::isAvailable(true)) SKIP("Test needs access to the NGSD test database!");
		...
		EXECUTE("NGSDAddVariantsGermline", "-test -debug -no_time -ps NA12878_45 -sv " + TESTDATA("data_in/NGSDAddVariantsGermline_in3.bedpe"));
		REMOVE_LINES(lastLogFile(), QRegularExpression("^filename:"));
		COMPARE_FILES(lastLogFile(), TESTDATA("data_out/NGSDAddVariantsGermline_out6.log"));
		...
		REMOVE_LINES(lastLogFile(), QRegularExpression("^filename:"));
		COMPARE_FILES(lastLogFile(), TESTDATA("data_out/NGSDAddVariantsGermline_out7.log"));
	...
	TEST_METHOD(import_with_existing_report_config)
	{
		if (!NGSD::isAvailable(true)) SKIP("Test needs access to the NGSD test database!");
		...
		REMOVE_LINES(lastLogFile(), QRegularExpression("^filename:"));
		COMPARE_FILES(lastLogFile(), TESTDATA("data_out/NGSDAddVariantsGermline_out10.log"));

		EXECUTE("NGSDAddVariantsGermline", "-test -debug -no_time -ps NA12878_45 -sv " + TESTDATA("data_in/NGSDAddVariantsGermline_in3.bedpe"));
		REMOVE_LINES(lastLogFile(), QRegularExpression("^filename:"));
		COMPARE_FILES(lastLogFile(), TESTDATA("data_out/NGSDAddVariantsGermline_out11.log"));
	...
	TEST_METHOD(sv_longread_import)
	{
		if (!NGSD::isAvailable(true)) SKIP("Test needs access to the NGSD test database!");
		...
		//check log
		REMOVE_LINES(lastLogFile(), QRegularExpression("^filename:"));
		COMPARE_FILES(lastLogFile(), TESTDATA("data_out/NGSDAddVariantsGermline_Test_line233.log"));
	}

	TEST_METHOD(re_import)
	{
		if (!NGSD::isAvailable(true)) SKIP("Test needs access to the NGSD test database!");
