...
private:
	...
	TEST_METHOD(test_01)
	{
		...
		COMPARE_FILES("out/FastqExtract_out1.fastq.gz", TESTDATA("data_out/FastqExtract_out1.fastq.gz"));
	}
	...
	TEST_METHOD(test_02)
	{
		...
		COMPARE_FILES("out/FastqExtract_out2.fastq.gz", TESTDATA("data_out/FastqExtract_out2.fastq.gz"));
	}

	TEST_METHOD(option_v)
	{
		...
		COMPARE_FILES("out/FastqExtract_out3.fastq.gz", TESTDATA("data_out/FastqExtract_out3.fastq.gz"));
	}

	TEST_METHOD(test_long_read)
	{
		...
		COMPARE_FILES("out/FastqExtract_out4.fastq.gz", TESTDATA("data_out/FastqExtract_out4.fastq.gz"));
	}
};
