...
private slots:
	...
	void ucsc_gene()
	{
		...
		//test
		EXECUTE("GenesToBed", "-test -in " + TESTDATA("data_in/GenesToBed_in1.txt") + " -out out/GenesToBed_out1.bed -source ucsc -mode gene");
	...
	void ucsc_exon()
	{
		...
		//test ucsc exon
		EXECUTE("GenesToBed", "-test -in " + TESTDATA("data_in/GenesToBed_in1.txt") + " -out out/GenesToBed_out2.bed -source ucsc -mode exon");
