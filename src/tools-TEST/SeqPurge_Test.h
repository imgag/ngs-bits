...
private slots:
	...
	void test_multithread()
	{
		...
		for (int i=1; i<=8; ++i)
		{
			...
			timer.restart();
			EXECUTE("SeqPurge", "-in1 " + TESTDATA("data_in/SeqPurge_in1.fastq.gz") + " -in2 " + TESTDATA("data_in/SeqPurge_in2.fastq.gz") + " -out1 "+out1+" -out2 "+out2+" -ncut 0 -qcut 0 -min_len 15 -qc out/SeqPurge_"+suffix+".qcML -summary out/SeqPurge_"+suffix+".log -threads " + QString::number(i));
