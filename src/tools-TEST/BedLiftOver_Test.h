...
private slots:
	...
	void test_01()
	{
		...
		EXECUTE("BedLiftOver", "-in " + TESTDATA("../cppNGS-Test/data_in/ChainFileReader_in1.bed") + " -chain " + bw_file + " -lifted out/BedLiftOver_out1_lifted.bed -unmapped out/BedLiftOver_out1_unmapped.bed -del 0.05");
	...
	void test_02()
	{
		...
		for (int i=0; i<4; i++)
		{
			EXECUTE("BedLiftOver", "-in " + TESTDATA("../cppNGS-Test/data_in/ChainFileReader_in3.bed") + " -chain " + bw_file + " -lifted out/BedLiftOver_out3_lifted.bed -unmapped out/BedLiftOver_out3_unmapped.bed -del " + QString::number(allowed_deletion[i], 'f'));
