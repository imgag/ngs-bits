...
public:
    ...
    virtual void setup()
    {
		...
		addInt("min_mapq", "Minimum mapping quality for re-calling.", true, 1);
	...
	VcfData loadVcf(QString filename, QList<VariantDefinition>& var_details, QHash<QByteArray, int>& var_tag_to_index, double min_qual, bool no_special_calls)
    {
        //init
        VcfData output;
        int c_snv_x = 0;
        ...
        while(!file.atEnd())
        {
            ...
            else //variant lines
            {
				...
				if (index==-1)
				{
					var_details << VariantDefinition{chr, pos, ref, alt, is_snv, tag};
    ...
    virtual void main()
    {
		...
		QList<VariantDefinition> var_details;
