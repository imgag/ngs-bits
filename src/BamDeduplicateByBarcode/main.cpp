...
private:
	...
	void writeDupCountHisto(QHash <int, BarcodeCountInfo> dup_count_histo, QTextStream &outStream)
	{
		...
		int reads_total_before=0;
		int reads_total_after=0;
		...
		foreach(int dup_count, dup_counts)
		{
			...
			reads_total_after+=dup_count_histo[dup_count].group_count();
			reads_total_before+=dup_count_histo[dup_count].group_count()*dup_count;
		}

		foreach(int dup_count, dup_counts)
		{
			QString barcodes_percentage=QString::number(dup_count_histo[dup_count].group_count()*100.0/barcodes_total, 'f', 2);
			QString reads_percentage=QString::number(dup_count_histo[dup_count].group_count()*dup_count*100.0/reads_total_before, 'f', 2);
	...
	most_frequent_read_selection findHighestFreqRead(QList <readPair> readpairs)
	{
		...
		int all_read_count=0;
		QString max_seq;
		bool ambiguous_most_freq = false;
		...
		foreach(QString seq,readpair_seq_count.keys())
		{
			if ((readpair_seq_count[seq].size())>max_read_count)
			{
				...
				max_seq=seq;
				ambiguous_most_freq = false;
			}
			else if ((readpair_seq_count[seq].size()) == max_read_count)
			{
				ambiguous_most_freq = true;
			}
		...
		result.ambiguous_most_freq = ambiguous_most_freq;
...
public:
	...
	virtual void setup()
	{
		...
		addFlag("del_amb", "remove barcode families where multiple read are tied for most frequent count");
	...
	virtual void main()
	{
		...
		bool delete_ambigous_selection = getFlag("del_amb");
