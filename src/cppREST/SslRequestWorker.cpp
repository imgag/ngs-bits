#include "SslRequestWorker.h"

SslRequestWorker::SslRequestWorker(QSslConfiguration ssl_configuration, qintptr socket)
	:
...
void SslRequestWorker::run()
{
	...
	connect(ssl_socket, static_cast<sslFailed>(&QSslSocket::sslErrors), this, &SslRequestWorker::sslFailed);
	connect(ssl_socket, &QSslSocket::peerVerifyError, this, &SslRequestWorker::verificationFailed);
	connect(ssl_socket, &QSslSocket::encrypted, this, &SslRequestWorker::securelyConnected);
	connect(ssl_socket, &QSslSocket::disconnected, this, &SslRequestWorker::socketDisconnected);
...
void SslRequestWorker::handleConnection()
{
	qDebug() << "Secure connection has been established";
}

void SslRequestWorker::socketDisconnected()
{
	qDebug() << "Client has disconnected from the socket";
}

QString SslRequestWorker::intToHex(const int& input)
{
	return QString("%1").arg(input, 10, 16, QLatin1Char('0')).toUpper();
}

void SslRequestWorker::closeAndDeleteSocket(QSslSocket* socket)
{
...
void SslRequestWorker::sendResponseDataPart(QSslSocket* socket, QByteArray data)
{
...
void SslRequestWorker::sendEntireResponse(QSslSocket* socket, HttpResponse response)
{
...
void SslRequestWorker::finishPartialDataResponse(QSslSocket* socket)
{
