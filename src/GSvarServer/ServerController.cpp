	...
	else
	{
		...
		switch(requested_type)
		{
			...
			case PathType::METHYLATION_TRACK:
				file_list << file_locator->getMethylationTrackFile();
				break;
			case PathType::METHYLATION_TRACK_HP1:
				file_list << file_locator->getMethylationTrackFileHP1();
				break;
			case PathType::METHYLATION_TRACK_HP2:
				file_list << file_locator->getMethylationTrackFileHP2();
				break;
			case PathType::PHASING_TRACK:
				file_list << file_locator->getPhasingTrackFile();
				break;
			case PathType::EPIGEN:
				file_list << file_locator->getEpigenFile();
				break;
			case PathType::GSVAR:
...
HttpResponse ServerController::performBlatSearch(const HttpRequest& request)
{
	...
	QProcess process;
	process.setProcessChannelMode(QProcess::MergedChannels);
	QString tmp_tsv = Helper::tempFileName(".tsv");
	Log::info("Running megSAP tool mapping_blatlike.php for sequence: " + sequence);
	process.start("php", QStringList() << PipelineSettings::rootDir() + "/src/Tools/mapping_blatlike.php" << "-seq" << sequence << "-out" << tmp_tsv);
	bool success = process.waitForFinished(-1);	
	if (!success || process.exitCode()>0)
	{
		return HttpResponse(ResponseStatus::INTERNAL_SERVER_ERROR, HttpUtils::detectErrorContentType(request.getHeaderByName("User-Agent")), EndpointManager::formatResponseMessage(request, QString("Error while executing mapping_blatlike.php: " + process.readAll())));
	}

    // reading results
	QSharedPointer<QFile> out_file = Helper::openFileForReading(tmp_tsv, false);
	QByteArray tsv_data = out_file->readAll();	

    BasicResponseData response_data;
	response_data.length = tsv_data.length();
	response_data.content_type = ContentType::TEXT_PLAIN;
	return HttpResponse(response_data, tsv_data);
}
