...
void SgeStatusUpdateWorker::run()
{
	try
	{
        if (debug_) QTextStream(stdout) << "SGE update started" << Qt::endl;
		...
		while(query.next())
		{
			...
			{
                if (debug_) QTextStream(stdout) << "SGE job (id=" << QString::number(job_id) << ") update failed: " << e.message() << Qt::endl;
				Log::info("SGE job (id=" + QString::number(job_id) + ") update failed: " + e.message());
			}
			catch (...)
			{
                if (debug_) QTextStream(stdout) << "SGE job (id=" << QString::number(job_id) << ") update failed with unkown error" << Qt::endl;
        ...
        if (debug_) QTextStream(stdout) << "SGE update done" << Qt::endl;
	}
	catch (Exception& e)
	{
        if (debug_) QTextStream(stdout) << "SGE update failed: " << e.message() << Qt::endl;
		Log::info("SGE status update failed: " + e.message());
	}
	catch (...)
	{
        if (debug_) QTextStream(stdout) << "SGE update failed with unkown error" << Qt::endl;
...
{
    if (debug_) QTextStream(stdout) << "Updating status of job " << job_id << " (type: " << job.type << " SGE-id: " << job.sge_id << ")" << Qt::endl;
