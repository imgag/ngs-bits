...
HttpResponse EndpointHandler::calculateStatsAvgCoverage(const HttpRequest& request)
{
	...
	QList<QString> url_parts = bam_file.split("/");

	QString url_id = (url_parts.count()>1 ? url_parts[url_parts.count()-2] : "");

	UrlEntity url_entity = UrlManager::getURLById(url_id);

	if (url_entity.filename_with_path.isEmpty())
	{
		return HttpResponse(ResponseStatus::INTERNAL_SERVER_ERROR, ContentType::TEXT_HTML, "Could not locate file specified in the URL");
	}
...
//		qDebug() << json_regions[i].toObject().value("chr").toString() << " - " << json_regions[i].toObject().value("start").toInt() << " -- " << json_regions[i].toObject().value("end").toInt();
		...
		while (reader.getNextAlignment(al))
		{
			...
			const int ol_start = std::max(region_start, al.start());
			const int ol_end = std::min(region_end, al.end());
			if (ol_start<=ol_end)
			{
				cov += ol_end - ol_start + 1;
			}
		}
		json_array.append(QString::number(cov));
	}
...
QString EndpointHandler::createFileTempUrl(const QString& file, const bool& return_http)
{
	...
	qDebug() << ServerHelper::getUrlProtocol(return_http) + ServerHelper::getStringSettingsValue("server_host") +
