...
public:
    ...
    virtual void main()
    {
        ...
        while (!in_p->atEnd())
        {
            ...
            if (reg != "")
            {
				QByteArray chr = VcfFileCheck::getPartByColumn(line, VcfFileCheck::CHROM);
				QByteArray start = VcfFileCheck::getPartByColumn(line, VcfFileCheck::POS);
				QByteArray ref = VcfFileCheck::getPartByColumn(line, VcfFileCheck::REF);
            ...
            if (variant_type != "")
			{
				QByteArray ref = VcfFileCheck::getPartByColumn(line, VcfFileCheck::REF).trimmed();
				QByteArray alt = VcfFileCheck::getPartByColumn(line, VcfFileCheck::ALT).trimmed();
            ...
            if (quality != 0.0)
            {
				QByteArray qual = VcfFileCheck::getPartByColumn(line, VcfFileCheck::QUAL).trimmed();
				bool ok;
            ...
            if (filter_empty)
            {
				QByteArray filter = VcfFileCheck::getPartByColumn(line, VcfFileCheck::FILTER).trimmed();
			...
			if (id != "")
            {
				QByteArray id = VcfFileCheck::getPartByColumn(line, VcfFileCheck::ID).trimmed();
            ...
            {
				QByteArrayList info_parts = VcfFileCheck::getPartByColumn(line, VcfFileCheck::INFO).trimmed().split(';');
            ...
            {
				QByteArrayList format_entries = VcfFileCheck::getPartByColumn(line, VcfFileCheck::FORMAT).trimmed().split(':');

				int samples_passing = 0;
				int samples_failing = 0;
				for (int i = VcfFileCheck::MIN_COLS + 1; i < column_count; ++i)
				{
					QByteArrayList sample_parts = VcfFileCheck::getPartByColumn(line, i).split(':');
