...
FilterWidgetFusion::FilterWidgetFusion(QWidget *parent)
	: QWidget(parent)
	, ui_()
...
{
	ui_.setupUi(this);
	ui_.cascade_widget->setSubject(VariantType::FUSIONS);
	...
	connect(action, &QAction::triggered, this, &FilterWidgetFusion::clearTargetRegion);
...
void FilterWidgetFusion::resetSignalsUnblocked(bool clear_roi)
{
	//filters
...
QString FilterWidgetFusion::filterFileName() const
{
	return GSvarHelper::applicationBaseName() + "_filters_fusion.ini";
}

void FilterWidgetFusion::reset(bool clear_roi)
{
	blockSignals(true);
...
void FilterWidgetFusion::markFailedFilters()
{
	ui_.cascade_widget->markFailedFilters();
}

bool FilterWidgetFusion::setTargetRegionByName(QString name)
{
	return FilterWidgetHelper::setTargetRegionByName(name, ui_.roi);
}

void FilterWidgetFusion::updateStateFilterName()
{
...
void FilterWidgetFusion::updateGuiFilterName()
{
...
void FilterWidgetFusion::updateStateFilterCascade()
{
	state_.setFilterCascade(ui_.cascade_widget->filters(), false);
}

void FilterWidgetFusion::updateGuiFilterCascade()
{
	ui_.cascade_widget->setFilters(state_.getFilterCascade());
}

void FilterWidgetFusion::updateStateTargetRegionFilter(int index)
{
...
void FilterWidgetFusion::updateGuiTargetRegionFilter()
{
	FilterWidgetHelper::setTargetRegionByName(state_.getTargetRegionInfo().name, ui_.roi);
}

void FilterWidgetFusion::updateStateRegionFilter()
{
...
void FilterWidgetFusion::updateGuiRegionFilter()
{
	ui_.region->setText(state_.getRegionFilter().toString(true));
}

void FilterWidgetFusion::updateStateGeneFilter()
{
	state_.setGenes(GeneSet::createFromText(ui_.gene->displayText().toUtf8(), ','), false);
}

void FilterWidgetFusion::updateGuiGeneFilter()
{
	ui_.gene->setText(state_.getGenes().toString(", "));
}

void FilterWidgetFusion::updateStateTextFilter()
{
	state_.setTextFilter(ui_.text->displayText(), false);
}

void FilterWidgetFusion::updateGuiTextFilter()
{
	ui_.text->setText(state_.getTextFilter());
}

void FilterWidgetFusion::updateStateReportConfigfilter()
{
...
void FilterWidgetFusion::updateGuiReportConfigfilter()
{
...
void FilterWidgetFusion::updateGuiPhenotypes()
{
...
void FilterWidgetFusion::editPhenotypes()
{
	//edit
...
void FilterWidgetFusion::showPhenotypeContextMenu(QPoint pos)
{
	//set up
	QMenu menu;
...
void FilterWidgetFusion::showRoiContextMenu(QPoint pos)
{
	//set up
	QMenu menu;
	QMenu* history_menu = menu.addMenu("history");
...
void FilterWidgetFusion::setFilter(int index)
{
	if (index==0)
	{
		ui_.cascade_widget->clear();
...
void FilterWidgetFusion::clearTargetRegion()
{
	ui_.roi->setCurrentText("none");
}

void FilterWidgetFusion::loadFilters()
{
