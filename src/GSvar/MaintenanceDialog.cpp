...
void MaintenanceDialog::deleteUnusedVariants()
{
    ...
    QSet<int> var_ids_pub = NGSHelper::listToSet(db.getValuesInt("SELECT variant_id FROM variant_publication WHERE variant_table='variant'"));
...
void MaintenanceDialog::importStudySamples()
{
	...
	foreach(const QString& study, studies_genlab)
	{
		...
		QStringList errors;
        QSet<int> genlab_ps_ids = NGSHelper::listToSet(genlab.studySamples(study, errors));
        ...
        QSet<int> extra_ids = NGSHelper::listToSet(db.getValuesInt("SELECT processed_sample_id FROM study_sample WHERE study_id='" + study_id + "'"));
...
void MaintenanceDialog::replaceObsolteHPOTerms()
{
	//init
	NGSD db;
    QSet<QString> hpo_terms_valid = NGSHelper::listToSet(db.getValues("SELECT hpo_id FROM hpo_term"));
    QSet<QString> hpo_terms_obsolete = NGSHelper::listToSet(db.getValues("SELECT hpo_id FROM hpo_obsolete"));
...
void MaintenanceDialog::deleteDataOfMergedSamples()
{
	...
	c_deleted = 0;
    QSet<int> ps_with_qc = NGSHelper::listToSet(db.getValuesInt("SELECT processed_sample_id FROM processed_sample_qc"));
	...
	//delete KASP data
	c_deleted = 0;
    QSet<int> ps_with_kasp = NGSHelper::listToSet(db.getValuesInt("SELECT processed_sample_id FROM kasp_status"));
...
void MaintenanceDialog::compareStructureOfTestAndProduction()
{
    ...
    QSet<QString> tables_both = NGSHelper::listToSet(tables_p).intersect(NGSHelper::listToSet(tables_t));
	...
	foreach(QString table, tables_both)
	{
        ...
        QSet<QString> fields_both = NGSHelper::listToSet(fields_p).intersect(NGSHelper::listToSet(fields_t));
...
QSet<int> MaintenanceDialog::psWithVariants(NGSD& db)
{
    ...
    ps_with_vars += NGSHelper::listToSet(db.getValuesInt("SELECT DISTINCT processed_sample_id FROM detected_variant"));
    ps_with_vars += NGSHelper::listToSet(db.getValuesInt("SELECT processed_sample_id FROM cnv_callset"));
    ps_with_vars += NGSHelper::listToSet(db.getValuesInt("SELECT processed_sample_id FROM sv_callset"));
    ps_with_vars += NGSHelper::listToSet(db.getValuesInt("SELECT processed_sample_id FROM re_callset"));
...
void MaintenanceDialog::clearUnusedReportConfigs(NGSD& db)
{
    ...
    used_rc_ids += NGSHelper::listToSet(db.getValuesInt("SELECT DISTINCT report_configuration_id FROM report_configuration_variant"));
    used_rc_ids += NGSHelper::listToSet(db.getValuesInt("SELECT DISTINCT report_configuration_id FROM report_configuration_cnv"));
    used_rc_ids += NGSHelper::listToSet(db.getValuesInt("SELECT DISTINCT report_configuration_id FROM report_configuration_sv"));
    used_rc_ids += NGSHelper::listToSet(db.getValuesInt("SELECT DISTINCT report_configuration_id FROM report_configuration_re"));
    used_rc_ids += NGSHelper::listToSet(db.getValuesInt("SELECT DISTINCT report_configuration_id FROM report_configuration_other_causal_variant"));
