...
bool GSvarHelper::queueSampleAnalysis(AnalysisType type, const QList<AnalysisJobSample>& samples, QWidget *parent)
{
	...
	if (!NGSD().userCanPerformAction(LoginManager::userId(), ActionPermission::START_ANALYSIS_JOBS))
	{
		QMessageBox::information(parent, "Acces denied", "You do not have permissions to perform this action!");
		return false;
	}
