...
RtfTable SomaticRnaReport::fusions()
{
	...
	fusion_table.addRow(RtfTableRow("Fusionen", 9636, RtfParagraph().setHorizontalAlignment("c").setBold(true)).setHeader().setBackgroundColor(1));

	fusion_table.addRow(RtfTableRow({"Fusion", "Transkript links", "Aminosäure linkes Gen", "Transkript rechts", "Aminosäure rechtes Gen", "Fusionstyp"},{2000,1800,1200,1800,1200,1636}, RtfParagraph().setBold(true).setHorizontalAlignment("c")).setHeader());
...
RtfTable SomaticRnaReport::snvTable()
{
	...
	for(int i=0; i<dna_snvs_.count(); ++i)
	{
		...
		row.addCell({trans.hgvs_c + ":" + trans.hgvs_p, RtfText(trans.id).setFontSize(14).RtfCode()}, 2750);
	...
	table.prependRow(RtfTableRow({"Gen", "Veränderung", "Typ", "Anteil", "Beschreibung", "Anteil", "TPM", "MW TPM*"},{1000,2750,1150,750,1750,675,625,936}, RtfParagraph().setBold(true).setHorizontalAlignment("c")).setHeader().setBorders(1, "brdrhair", 2) );
	table.prependRow(RtfTableRow({"DNA", "RNA"}, {7400, 2236}, RtfParagraph().setHorizontalAlignment("c").setBold(true)).setBorders(1, "brdrhair", 2) );
	table.prependRow(RtfTableRow("Punktmutationen (SNVs) und kleine Insertionen/Deletionen (INDELs)", 9636, RtfParagraph().setHorizontalAlignment("c").setBold(true)).setHeader().setBackgroundColor(1).setBorders(1, "brdrhair", 2) );
	...
	desc += RtfText("*MW TPM:").setBold(true).setFontSize(14).RtfCode() + " " + "Mittelwerte in Transcripts per Million (TPM) von Vergleichsproben aus " + ref_tissue_type_ + " (The Human Protein Atlas). ";
...
RtfTable SomaticRnaReport::cnvTable()
{
	...
	RtfTable table;
	table.addRow(RtfTableRow("Kopienzahlveränderungen (CNVs)", 9636, RtfParagraph().setHorizontalAlignment("c").setBold(true).setFontSize(16)).setHeader().setBackgroundColor(1).setBorders(1, "brdrhair", 2) );
	table.addRow(RtfTableRow({"Gen", "Position", "CNV", "CN", "Anteil","Beschreibung","TPM RNA", "MW TPM*"},{1000,2300,1300,500,750,2286,750,750}, RtfParagraph().setFontSize(16).setBold(true).setHorizontalAlignment("c")).setHeader());
	...
	for(int i=0; i<dna_cnvs_.count(); ++i)
	{
		...
		for(int j=0; j<statements.count(); ++j)
		{
			if(statements[j].contains("known") || statements[j].contains("driver"))
			{
				...
				temp.addCell(2286, statements[j], RtfParagraph().setFontSize(16));
				temp.addCell(750, QByteArray::number(getTpm(genes[j], cnv.annotations().at(i_rna_tpm)), 'f', 1), RtfParagraph().setFontSize(16).setHorizontalAlignment("c") );
				temp.addCell(750, QByteArray::number(getTpm(genes[j], cnv.annotations().at(i_ref_rna_tpm)), 'f', 1), RtfParagraph().setFontSize(16).setHorizontalAlignment("c") );
	...
	RtfSourceCode desc = "";
	desc += RtfText("*MW TPM:").setBold(true).RtfCode() + " " + "Mittelwerte von Vergleichsproben aus " + ref_tissue_type_ + " (The Human Protein Atlas).";
