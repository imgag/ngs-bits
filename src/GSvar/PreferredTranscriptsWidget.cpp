...
void PreferredTranscriptsWidget::check()
{
	...
	for(auto it=pts_by_gene_id.begin(); it!=pts_by_gene_id.end(); ++it)
	{
        ...
        QSet<QString> mane = NGSHelper::listToSet(db.getValues("SELECT name FROM gene_transcript WHERE source='Ensembl' AND gene_id=" + QString::number(gene_id) + " AND is_mane_select=1"));
		if (mane.isEmpty()) continue;

        QSet<QString> mane_not_pt = mane.subtract(NGSHelper::listToSet(it.value()));
