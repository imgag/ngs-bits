...
void MainWindow::applyFilters(bool debug_time)
{
	try
	{
		...
		{
            QSet<int> report_variant_indices = NGSHelper::listToSet(report_settings_.report_config->variantIndices(VariantType::SNVS_INDELS, false));
			for(int i=0; i<variants_.count(); ++i)
			{
		...
		{
            QSet<int> report_variant_indices = NGSHelper::listToSet(somatic_report_settings_.report_config.variantIndices(VariantType::SNVS_INDELS, false));
			for(int i=0; i<variants_.count(); ++i)
			{
				if ( !filter_result_.flags()[i] ) continue;
