...
ReportDialog::ReportDialog(AnalysisDataController& data_controller, QWidget* parent)
	: QDialog(parent)
	, ui_()
	, ps_(data_controller.germlineReportSample())
    , data_controller_(data_controller)
{
	ui_.setupUi(this);
	setWindowTitle(windowTitle() + ps_);
...
void ReportDialog::checkMetaData()
{
	...
	QHash<QString, QStringList> errors = db_.checkMetaData(ps_id, data_controller_.getSmallVariantList(), data_controller_.getCnvList(), data_controller_.getSvList(), data_controller_.getReList());
...
void ReportDialog::initGUI()
{
	...
	//settings
	const ReportSettings& settings = data_controller_.getGermlineReportSettings();
	ui_.details_cov->setChecked(settings.show_coverage_details);
	ui_.min_cov->setValue(settings.min_depth);
	ui_.cov_based_on_complete_roi->setChecked(settings.cov_based_on_complete_roi);
	ui_.cov_exon_padding->setValue(settings.cov_exon_padding);
	ui_.depth_calc->setChecked(settings.recalculate_avg_depth);
	ui_.omim_table->setChecked(settings.show_omim_table);
	ui_.omim_table_one_only->setChecked(settings.show_one_entry_in_omim_table);
	ui_.class_info->setChecked(settings.show_class_details);
	ui_.language->setCurrentText(settings.language);
	ui_.refseq_trans_names->setChecked(settings.show_refseq_transcripts);
	...
	if (data_controller_.getAnalysisType() == AnalysisType::GERMLINE_MULTISAMPLE)
	{
		QStringList ps_list = data_controller_.getSmallVariantList().getSampleHeader().sampleNames().values();
		ps_list.sort();
		ps_list.removeAll("");
...
void ReportDialog::updateVariantTable()
{
	...
	const TargetRegionInfo& roi = data_controller_.getSmallVariantsFilterState().getTargetRegionInfo();
	const ReportSettings& settings = data_controller_.getGermlineReportSettings();

	int geno_idx = variants.getSampleHeader().infoByID(ps_).column_index;
	int gene_idx = variants.annotationIndexByName("gene");
	int class_idx = variants.annotationIndexByName("classification");
	foreach(int i, settings.report_config->variantIndices(VariantType::SNVS_INDELS, true, type()))
	{
		Variant variant = variants[i];
		const ReportVariantConfiguration& var_conf = settings.report_config->get(VariantType::SNVS_INDELS, i);

		//manual curation
		if (var_conf.isManuallyCurated()) var_conf.updateVariant(variant, genome_idx, geno_idx);

		bool in_roi = true;
		if (roi.isValid() && !roi.regions.overlapsWith(variant.chr(), variant.start(), variant.end())) in_roi = false;
	...
	//add CNVs
	const CnvList& cnvs = data_controller_.getCnvList();
	foreach(int i, settings.report_config->variantIndices(VariantType::CNVS, true, type()))
	{
		CopyNumberVariant cnv = cnvs[i];
		const ReportVariantConfiguration& var_conf = settings.report_config->get(VariantType::CNVS,i);

		//manual curation
		if (var_conf.isManuallyCurated()) var_conf.updateCnv(cnv, cnvs.annotationHeaders(), db_);

		bool in_roi = true;
		if (roi.isValid() && !roi.regions.overlapsWith(cnv.chr(), cnv.start(), cnv.end())) in_roi = false;

		ui_.vars->setRowCount(ui_.vars->rowCount()+1);
		addCheckBox(row, 0, in_roi, !in_roi)->setData(Qt::UserRole, i);
		addTableItem(row, 1, var_conf.report_type + (var_conf.causal ? " (causal)" : ""));
		addTableItem(row, 2, variantTypeToString(VariantType::CNVS));
		addTableItem(row, 3, cnv.toStringWithMetaData() + " cn=" + QString::number(cnv.copyNumber(cnvs.annotationHeaders())));
	...
	//add SVs
	const BedpeFile& svs = data_controller_.getSvList();
	if (svs.isValid())
	{
		...
		if (svs.format() == BedpeFileFormat::BEDPE_GERMLINE_MULTI || svs.format() == BedpeFileFormat::BEDPE_GERMLINE_TRIO)
		{
			try
			{
				sv_sample_idx = svs.sampleHeaderInfo().infoByStatus(true).column_index - svs.annotationIndexByName("FORMAT") - 1; //relativ to column after FORMAT
			}
			catch (...)
			{
				sv_sample_idx = -1;
		...
		foreach(int i, settings.report_config->variantIndices(VariantType::SVS, true, type()))
		{
			BedpeLine sv = svs[i];
			const ReportVariantConfiguration& var_conf = settings.report_config->get(VariantType::SVS,i);

			//manual curation
			if (var_conf.isManuallyCurated()) var_conf.updateSv(sv, svs.annotationHeaders(), db_);

			QByteArray genotype = "n/a";
			if (sv_sample_idx!=-1) genotype = sv.genotypeHumanReadable(svs.annotationHeaders(), false, sv_sample_idx);

			//check if variant is in ROI (if there is a ROI)
			bool in_roi = true;
			if (roi.name!="")
			{
				in_roi = sv.affectedRegion().overlapsWith(roi.regions);
			}
			...
			addTableItem(row, 4, sv.genes(svs.annotationHeaders()).join(", "));
			addTableItem(row, 5, var_conf.classification);
			++row;
		}
	}
	...
	//add REs
	const RepeatLocusList& res = data_controller_.getReList();
	foreach(int i, settings.report_config->variantIndices(VariantType::RES, true, type()))
	{
		RepeatLocus re = res[i];
		const ReportVariantConfiguration& var_conf = settings.report_config->get(VariantType::RES, i);
		...
		if (roi.name!="")
		{
			in_roi = roi.regions.overlapsWith(re.region());
		}
	...
	OtherCausalVariant causal_variant = settings.report_config->otherCausalVariant();
...
{
	VariantList missing_small_variants = data_controller_.validateGermlineReportSmallVariants();
	CnvList missing_cnvs = data_controller_.validateGermlineReportCnvs();
	BedpeFile missing_svs = data_controller_.validateGermlineReportSvs();
	RepeatLocusList missing_res = data_controller_.validateGermlineReportRes();
	...
	if (missing_res.count() > 0)
	{
		message_text << "Res: ";
		for (int i = 0; i < missing_res.count(); ++i)
		{
			message_text << "\t" + missing_res[i].toString(true, true);
		}
		message_text << "";
	}
...
{
	ReportSettings& settings = data_controller_.getGermlineReportSettings();
	settings.selected_variants.clear();
	settings.select_other_causal_variant = false;
	for (int r=0; r<ui_.vars->rowCount(); ++r)
	{
		...
		if(index != -1)
		{
			...
			settings.selected_variants << qMakePair(type, index);
		}
		else
		{
			// other causal variant
			settings.select_other_causal_variant = true;
		}
	}

	settings.show_coverage_details = ui_.details_cov->isChecked();
	settings.min_depth = ui_.min_cov->value();
	settings.cov_based_on_complete_roi = ui_.cov_based_on_complete_roi->isChecked();
	settings.cov_exon_padding = ui_.cov_exon_padding->value();
	settings.recalculate_avg_depth = ui_.depth_calc->isChecked();
	settings.show_omim_table = ui_.omim_table->isChecked();
	settings.show_one_entry_in_omim_table = ui_.omim_table_one_only->isChecked();
	settings.show_class_details = ui_.class_info->isChecked();
	settings.language = ui_.language->currentText();
	settings.show_refseq_transcripts = ui_.refseq_trans_names->isChecked();

	for (int row=0; row<ui_.add_samples->rowCount(); ++row)
	{
		const QTableWidgetItem* item = ui_.add_samples->item(row, 0);
		if (item->checkState()==Qt::Checked) settings.ps_additional << item->text();
	}
