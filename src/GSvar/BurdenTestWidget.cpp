...
void BurdenTestWidget::validateInputData()
{
    ...
    overlap = NGSHelper::listToSet(sample_ids_cases.keys()) & NGSHelper::listToSet(sample_ids_controls.keys());
	if(overlap.size() > 0)
	{
	...
	foreach (int s_id, sample_ids_cases.keys())
	{
		...
		{
            QSet<int> ps_ids = NGSHelper::listToSet(sample_ids_cases.value(s_id));
			QStringList ps_list;
	...
	foreach (int s_id, sample_ids_controls.keys())
	{
		...
		{
            QSet<int> ps_ids = NGSHelper::listToSet(sample_ids_controls.value(s_id));
			QStringList ps_list;
			foreach (int id, ps_ids) ps_list << db_.processedSampleName(QString::number(id));
			int ps_id = getNewestProcessedSample(ps_ids);
			ps_ids.remove(ps_id);
	...
	foreach (int s_id, sample_ids_cases.keys())
	{
        ...
        QSet<int> same_sample_overlap = same_samples & NGSHelper::listToSet(sample_ids_cases.keys());
        ...
        same_sample_overlap = same_samples & NGSHelper::listToSet(sample_ids_controls.keys());
	...
	foreach (int s_id, sample_ids_controls.keys())
	{
		...
		//add sample itself
		same_samples.insert(s_id);
        QSet<int> same_sample_overlap = same_samples & NGSHelper::listToSet(sample_ids_controls.keys());
		if (same_sample_overlap.size() > 1)
		{
			//add sample itself
	...
	foreach (int s_id, sample_ids_cases.keys())
	{
		...
		{
            QSet<int> related_sample_overlap = db_.relatedSamples(s_id, relation) &  NGSHelper::listToSet(sample_ids_cases.keys());
            ...
            related_sample_overlap = db_.relatedSamples(s_id, relation) &  NGSHelper::listToSet(sample_ids_controls.keys());
			if (related_sample_overlap.size() > 1)
			{
				QStringList related_sample_list;
				foreach (int id, related_sample_overlap)
	...
	foreach (int s_id, sample_ids_controls.keys())
	{
		...
		foreach (const QString& relation, relation_types)
		{
            QSet<int> related_sample_overlap = db_.relatedSamples(s_id, relation) &  NGSHelper::listToSet(sample_ids_controls.keys());
			if (related_sample_overlap.size() > 1)
			{
				QStringList related_sample_list;
				foreach (int id, related_sample_overlap)
    ...
    if (processing_systems.size() > 1) warnings << "WARNING: The cohort contains multiple processing systems (" + processing_systems.values().join(", ")+ ")!";
    if (processing_systems.size() > 1) warnings << "WARNING: The cohort contains multiple ancestries (" + ancestry.values().join(", ")+ ")!";
	...
	foreach(int s_id, sample_ids_cases.keys())
	{
        case_samples_to_keep += NGSHelper::listToSet(sample_ids_cases.value(s_id));
	}
	foreach(int s_id, sample_ids_controls.keys())
	{
        control_samples_to_keep += NGSHelper::listToSet(sample_ids_controls.value(s_id));
	}
...
void BurdenTestWidget::performBurdenTest()
{
	...
	foreach (int gene_id, gene_ids)
	{
		...
		if (include_cnvs)
		{
			...
			//get combined counts
            int n_cases_combined = (NGSHelper::listToSet(ps_names_cases) + NGSHelper::listToSet(ps_names_cases_cnv)).size();
            int n_controls_combined = (NGSHelper::listToSet(ps_names_controls) + NGSHelper::listToSet(ps_names_controls_cnv)).size();
...
int BurdenTestWidget::countOccurences(const QSet<int>& variant_ids, const QSet<int>& ps_ids, const QMap<int, QSet<int>>& detected_variants, Inheritance inheritance, QStringList& ps_names)
{
	...
	foreach(int ps_id, ps_ids)
	{
		...
		if(inheritance == Inheritance::de_novo)
		{
            ...
            QSet<int> de_novo_var_ids = NGSHelper::listToSet(db_.getValuesInt("SELECT variant_id FROM report_configuration_variant WHERE de_novo=TRUE AND report_configuration_id=:0", QString::number(rc_id)));
