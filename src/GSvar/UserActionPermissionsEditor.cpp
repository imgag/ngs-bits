	...
	, table_display_name_(table.replace("_", " "))
{
	...
	connect(ui_.sample_search_cb, SIGNAL(stateChanged(int)), this, SLOT(updateTable()));

	NGSD db;
	SqlQuery query = db.getQuery();
	query.exec("SELECT read_only, perform_variant_search, perform_burden_test, start_analysis_jobs, sample_search FROM user_action_permissions WHERE user_id='" + user_id_ + "'");
	while(query.next())
	{
		...
		if (query.value(4).toBool()) ui_.sample_search_cb->setChecked(true);
	}
}
...
void UserActionPermissionsEditor::updateTable()
{
	...
	try
	{
		...
		if (!ui_.read_only_cb->isChecked() && !ui_.variant_search_cb->isChecked() && !ui_.burden_test_cb->isChecked() && !ui_.start_analysis_jobs_cb->isChecked() && !ui_.sample_search_cb->isChecked())
		{
		...
		{
			db.getQuery().exec("INSERT INTO user_action_permissions (user_id, read_only, perform_variant_search, perform_burden_test, start_analysis_jobs, sample_search) VALUES ('" + user_id_ + "','"+QString::number(ui_.read_only_cb->isChecked()) + "','"+QString::number(ui_.variant_search_cb->isChecked())+"', '"+QString::number(ui_.burden_test_cb->isChecked())+"', '"+QString::number(ui_.start_analysis_jobs_cb->isChecked())+"', '"+QString::number(ui_.sample_search_cb->isChecked())+"')");
		}
		else
		{
			db.getQuery().exec("UPDATE user_action_permissions SET read_only='"+QString::number(ui_.read_only_cb->isChecked())+"', perform_variant_search='"+QString::number(ui_.variant_search_cb->isChecked())+"', perform_burden_test='"+QString::number(ui_.burden_test_cb->isChecked())+"', start_analysis_jobs='"+QString::number(ui_.start_analysis_jobs_cb->isChecked())+"', sample_search='"+QString::number(ui_.sample_search_cb->isChecked())+"' WHERE user_id='"+user_id_+ "'");
		}
	}
