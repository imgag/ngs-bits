...
public:
	...
	void importSmallVariants(NGSD& db, QTextStream& out, QString ps_name, bool debug, bool no_time, bool force, bool var_update)
	{
		...
		{
            out << "Skipped import because variants were already imported for '" + ps_name + "'. Use the flag '-force' to overwrite them." << Qt::endl;
			return;
		}
		...
		if (debug)
		{
            out << "DEBUG: Skipped " << variant_ids.count(-1) << " high-AF variants!" << Qt::endl;
		}
	...
	void importSVs(NGSD& db, QTextStream& out, QString ps_name, bool debug, bool no_time, bool force)
	{
		...
		{
            out << "NOTE: SVs were already imported for '" << ps_name << "' - skipping import" << Qt::endl;
			return;
		}
	...
	void importREs(NGSD& db, QTextStream& out, QString ps_name, bool debug, bool no_time, bool force)
	{
		...
		{
            out << "NOTE: REs were already imported for '" << ps_name << "' - skipping import" << Qt::endl;
			return;
		}
