...
void NGSHelper::annotateGermlineWithSomatic(VariantList& germline_snvs, const VariantList& somatic_snvs, const QByteArray& somatic_prefix, NGSD& db)
{
	//Make sure that only germline variant files are annotated with somatic pair files
	if(germline_snvs.type() != AnalysisType::GERMLINE_SINGLESAMPLE) return;
	if(somatic_snvs.type() != AnalysisType::SOMATIC_PAIR) return;

	//Get Indices
	int i_germline_gene = germline_snvs.annotationIndexByName("gene",true,false);
	int i_somatic_gene = somatic_snvs.annotationIndexByName("gene",true,false);

	int i_somatic_type = somatic_snvs.annotationIndexByName("variant_type",true,false);
	int i_somatic_af = somatic_snvs.annotationIndexByName("tumor_af",true,false);
	int i_somatic_dp = somatic_snvs.annotationIndexByName("tumor_dp",true,false);

	int i_somatic_cgi_driver_statement = somatic_snvs.annotationIndexByName("CGI_driver_statement",true,false);

	//Add empty annotation column
	int i_germline_annot_type = germline_snvs.addAnnotationIfMissing(somatic_prefix + "_somatic_variants","semicolon-separated SNPs in the same gene from the somatic file. genomic_alteration:variant_type:tumor_af:tumor_dp:CGI_driver_statement","");
	...
	for(int i=0;i<germline_snvs.count();++i)
	{
		...
		GeneSet germline_genes = db.genesToApproved(GeneSet::createFromText(germline_snvs[i].annotations().at(i_germline_gene),','),true);

		QByteArray annotation = "";

		for(int j=0;j<somatic_snvs.count();++j)
		{
			GeneSet somatic_genes = db.genesToApproved(GeneSet::createFromText(somatic_snvs[j].annotations().at(i_somatic_gene),','),true);
			...
			foreach(QByteArray gene, somatic_genes)
			{
				if(germline_genes.contains(gene))
				{
					...
					QByteArray pos = somatic_snvs[j].chr().str() + "_" + QByteArray::number(somatic_snvs[j].start()) + "_" + QByteArray::number(somatic_snvs[j].end()) + "_" + somatic_snvs[j].ref() + "_" + somatic_snvs[j].obs();
					annotation.append(pos + ":");

					annotation.append(somatic_snvs[j].annotations().at(i_somatic_type) + ":");
					annotation.append(somatic_snvs[j].annotations().at(i_somatic_af) + ":");
					annotation.append(somatic_snvs[j].annotations().at(i_somatic_dp));
					if(i_somatic_cgi_driver_statement != -1)
					{
						annotation.append(":" + somatic_snvs[j].annotations().at(i_somatic_cgi_driver_statement));
					}
		...
		germline_snvs[i].annotations()[i_germline_annot_type] = annotation;
	}
}
