...
bool VcfFileCheck::isValid(QString vcf_file_path, QString ref_file, QTextStream& out_stream, bool print_general_information, int max_lines)
{
	// determine file type:
	...
	foreach(KeyValuePair kvp, VcfFileCheck::INFO_URL_MAPPING)
	{
	...
	while(!eof && l<max_lines)
	{
		...
		else
		{
			...
			//chromosome
			Chromosome chr(parts[VcfFileCheck::CHROM]);
			if (chr.str().contains(":"))
			{
				printError(out_stream, "Chromosome '" + parts[VcfFileCheck::CHROM] + "' is not valid!", l, line);
				return false;
			}

			//position
			bool pos_is_valid;
			int pos = parts[VcfFileCheck::POS].toInt(&pos_is_valid);
			if(!pos_is_valid)
			{
				printError(out_stream, "Chromosomal position '" + parts[VcfFileCheck::POS] + "' is not a number!", l, line);
				return false;
			}

			//reference base
			QByteArray ref = parts[VcfFileCheck::REF].toUpper();
			if (pos_is_valid)
			{
			...
			//alternate base(s)
			QByteArrayList alts = parts[VcfFileCheck::ALT].split(',');
			...
			//quality
			const QByteArray& qual = parts[VcfFileCheck::QUAL];
			if (qual!=".")
			{
				bool ok = false;
				qual.toDouble(&ok);
			...
			//filter
			const QByteArray& filter = parts[VcfFileCheck::FILTER];
			...
			QByteArrayList format_names = parts[VcfFileCheck::FORMAT].split(':');
...
VcfFileCheck::DefinitionLine VcfFileCheck::parseDefinitionLine(QTextStream& out, int l, QByteArray line)
{
	if (!line.endsWith(">"))
	{
...
void VcfFileCheck::checkValues(const VcfFileCheck::DefinitionLine& def, const QByteArrayList& values, int alt_count, const QByteArray& sample, QTextStream& out, int l, const QByteArray& line)
{
...
QByteArray VcfFileCheck::getPartByColumn(const QByteArray& line, int index)
{
	int columns_seen = 0;
	int column_start = 0;
	int column_end = -1;
...
//Define URL encoding
const QList<KeyValuePair> VcfFileCheck::INFO_URL_MAPPING =
{
...
QString VcfFileCheck::encodeInfoValue(QString info_value)
{
...
QString VcfFileCheck::decodeInfoValue(QString encoded_info_value)
{
	// iterate over the mapping list in reverse order and replace each encoded character
	for (int i=VcfFileCheck::INFO_URL_MAPPING.size() - 1; i >= 0; i--)
	{
		encoded_info_value.replace(VcfFileCheck::INFO_URL_MAPPING[i].value, VcfFileCheck::INFO_URL_MAPPING[i].key);
	}
