...
HgvsNomenclature VariantHgvsAnnotator::variantToHgvs(const Transcript& transcript, VcfLine& variant, const FastaFileIndex& genome_idx)
{
    ...
    if(!variant.isSNV() && hgvs.hgvs_p != "")
    {
        ...
        else if(variant.isDel() && !hgvs.variant_consequence_type.contains(VariantConsequenceType::START_RETAINED_VARIANT))
        {
        ...
        {
            if(!hgvs.variant_consequence_type.contains(VariantConsequenceType::START_RETAINED_VARIANT))
            {
            ...
            (!plus_strand && gen_pos < transcript.codingEnd()))
    {
                ...
                (!plus_strand && gen_pos >= transcript.start()))
        {
            ...
            if(pos_hgvs_c.startsWith("+"))
            {
                ...
                pos_hgvs_c = "*" + QString::number(cds_length) + pos_hgvs_c;
            }
            else
            {
                pos_hgvs_c = "*" + pos_hgvs_c;
                ...
                && genomic_position < regions[i+1].start())
        {
            ...
            if(plus_strand)
            {
                ...
                if(utr_5)
                {
                    ...
                    {
                        pos_in_intron = "-" + QString::number(dist_below);
                    }
                    else
                    {
                        pos_in_intron = "+" + QString::number(dist_above);
                    }
                }
                else
                {
    ...
    if(!pos_found) //only happens in the case stated above, when regions are 5 or 3 prime utr
    {
        ...
        if(utr_5)
        {
            ...
            if(plus_strand)
            {
                ...
                {
                    pos_in_intron = "-1+" + QString::number(dist_above);
                }
            }
            else
            {
...
QString VariantHgvsAnnotator::getHgvsProteinAnnotation(const VcfLine& variant, const FastaFileIndex& genome_idx, const QString& pos_hgvs_c, const Transcript& transcript, bool plus_strand)
{
    ...
    else if(variant.isInDel())
    {
        ...
        else if(variant.isDel())
        {
            ...
            else if(aa_obs != toThreeLetterCode(NGSHelper::translateCodon(seq_ref.mid(frame_diff, 3))))
            {
