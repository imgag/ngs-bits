...
{
	return name() + " &ge; " + QString::number(getDouble("min_ll"), 'f', 2) + QString(getBool("scale_by_region")?" (scaled by region)":"");
}
...
void FilterCnvLoglikelihood::apply(const CnvList& cnvs, FilterResult& result) const
{
	...
	bool scale_by_region = getBool("scale_by_region");
	...
	if (cnvs.type()==CnvListType::CLINCNV_GERMLINE_SINGLE || cnvs.type()==CnvListType::CLINCNV_TUMOR_NORMAL_PAIR)
	{
		for(int i=0; i<cnvs.count(); ++i)
		{
			...
			if (scale_by_region)
			{
				double number_of_regions = cnvs[i].regions();
				if (number_of_regions <= 0.0) THROW(FileParseException, "Invalid number of regions!");
	...
	{
		if (scale_by_region) THROW(ArgumentException, "log-likelihood scaling cannot be applied to Multisample/Trio CNVs!");
		for(int i=0; i<cnvs.count(); ++i)
		{
			if (!result.flags()[i]) continue;
