...
void FilterTrio::apply(const VariantList& variants, FilterResult& result) const
{
    ...
    QSet<QString> types = NGSHelper::listToSet(getStringList("types"));
...
void FilterSvFilterColumn::apply(const BedpeFile& svs, FilterResult& result) const
{
    ...
    QSet<QString> filter_entries = NGSHelper::listToSet(getStringList("entries"));
	QString action = getString("action");
	...
	if (action=="REMOVE")
	{
		for(int i=0; i<svs.count(); ++i)
		{
            ...
            QSet<QString> sv_entries = NGSHelper::listToSet(QString(svs[i].annotations()[filter_col_index]).split(';'));
