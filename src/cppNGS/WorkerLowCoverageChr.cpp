...
void WorkerLowCoverageChr::run()
{
	try
	{
		...
		while (reader.getNextAlignment(al))
		{
			...
			min_baseq_>0 ? countCoverageWGSWithBaseQuality(min_baseq_, cov, al.start() - 1, al.end(), baseQualities, al) : countCoverageWGSWithoutBaseQuality(al.start()-1, al.end(), cov);
...
void WorkerLowCoverageChr::countCoverageWGSWithBaseQuality(
		int min_baseq,
		QVector<unsigned char>& cov,
		int start,
		int end,
...
void WorkerLowCoverageChr::countCoverageWGSWithoutBaseQuality(
		int start,
		int end,
		QVector<unsigned char>& cov)
{
